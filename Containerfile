...
RUN yum upgrade -y && yum upgrade -y && yum install epel-release -y && yum install sysstat -y && yum install tcpdump tcptraceroute wireshark sudo -y && yum install python3 -y && yum install smartmontools -y
